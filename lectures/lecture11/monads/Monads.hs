...
zipTree1 (Branch l1 r1) (Branch l2 r2) = 
  case zipTree1 l1 l2 of
    Nothing -> Nothing
    Just l' ->
      case zipTree r1 r2 ofNothing -> Nothing
...
InstanceSigs language extension. We do that here in the cabal file associated
with this project.)
...
-- >>= is bind: (>>= :: Monad m => m a -> (a -> m b) -> m b) 
-- return :: Monad m => a -> m a

-- extract a
fmapMonad :: (Monad m) => (a -> b) -> m a -> m b
-- fmapMonad f m = m >>= (\a -> return (f a))
...
-- zapMonad g m = m >>= \a -> g >>= \f -> return (f a)
zapMonad mf ma = g >>= \f -> ma >>= \a -> return (f a)
-- \f = grab f
...
pairs5 xs ys = [(x, y) | x <- xs, y <- ys, x /= y] -- /= is a guard
...
firstLess1 xs ys = do
  ...
  guard (x < y)
  return (x, y)
  -- if x < y then [(x, y)] else [] (replaced by guard + return)
...
-- "x is drawn from xs & f of x holds"
